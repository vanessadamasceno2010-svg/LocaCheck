...
function whatsappLocalBR(value) {
  const digits = onlyDigits(value);
  return digits.startsWith("55") ? digits.slice(2, 13) : digits.slice(0, 11);
}
function whatsappStoredBR(value) {
  const local = whatsappLocalBR(value);
  return local ? `55${local}` : "";
}
function money(value) { return Number(value || 0).toLocaleString("pt-BR", { style: "currency", currency: "BRL" }); }
const DEFAULT_ABOUT_TEXT = (name) => `A ${name || "sua locadora"} oferece locação de motos com atendimento próximo, informações claras e modelos preparados para diferentes necessidades. Nosso objetivo é facilitar sua locação com segurança, praticidade e transparência.`;

const DEFAULT_MOTO_DESCRIPTION = "Partida elétrica, injeção eletrônica, seguro, freio a disco, revisada.";
const emptyMoto = { name: "", model_year: "", engine_cc: "", deposit: "", description: DEFAULT_MOTO_DESCRIPTION, image_url: "", price_day: "", price_week: "", price_month: "", available: true, published: true, sort_order: 0 };
...
export default function MyRentalSiteModal({ userId, profile, onClose, onBuyCredits, showToast }) {
  ...
  const [form, setForm] = useState({
    ...
    hero_subtitle: "",
    about_title: "",
    about_text: DEFAULT_ABOUT_TEXT(profile?.nome || "sua locadora"),
    whatsapp: whatsappLocalBR(profile?.whatsapp || ""),
    phone: "",
    email: profile?.email || "",
    instagram: "",
    address: "",
    ...
    try {
      ...
      if (siteData) {
        setForm({
          ...
          logo_url: siteData.logo_url || "", hero_image_url: siteData.hero_image_url || heroFallback, hero_title: siteData.hero_title || "",
          hero_subtitle: siteData.hero_subtitle || "", about_title: siteData.about_title || `Sobre a ${siteData.brand_name || "sua locadora"}`, about_text: siteData.about_text || DEFAULT_ABOUT_TEXT(siteData.brand_name),
          whatsapp: whatsappLocalBR(siteData.whatsapp || profile?.whatsapp || ""), phone: siteData.phone || "", email: siteData.email || profile?.email || "",
  ...
  async function handleCreate(event) {
    ...
    setSaving(true);
    try {
      const created = await createMyRentalSite({ userId, brandName: form.brand_name, slug: form.slug, tagline: form.tagline, whatsapp: whatsappStoredBR(form.whatsapp), email: form.email });
  ...
  async function saveSite() {
    ...
    try {
      const updated = await updateMyRentalSite(site.id, { ...form, whatsapp: whatsappStoredBR(form.whatsapp), slug: validation.slug, tagline: form.tagline.trim() || null, hero_title: form.hero_title.trim() || null, hero_subtitle: form.hero_subtitle.trim() || null, about_title: form.about_title.trim() || null, about_text: form.about_text.trim() || null });
  ...
  async function handlePublishToggle() {
    ...
    if (!site.published && whatsappLocalBR(form.whatsapp).length < 10) { setMessage("Informe um WhatsApp válido antes de publicar o site."); setTab("contato"); return; }
  ...
  async function saveMoto(event) {
    ...
    if (!site?.id || !active) return;
    try {
      const payload = { ...motoForm, model_year: motoForm.model_year ? Number(motoForm.model_year) : null, engine_cc: motoForm.engine_cc ? Number(motoForm.engine_cc) : null, deposit: Number(motoForm.deposit || 0), price_day: Number(motoForm.price_day || 0), price_week: Number(motoForm.price_week || 0), price_month: Number(motoForm.price_month || 0), sort_order: Number(motoForm.sort_order || 0) };
  ...
  function editMoto(moto) { setEditingMotoId(moto.id); setMotoForm({ ...moto, model_year: moto.model_year || "", engine_cc: moto.engine_cc || "", deposit: moto.deposit || "", price_day: moto.price_day || "", price_week: moto.price_week || "", price_month: moto.price_month || "" }); setTab("frota"); }
  ...
  const tabs = [
    ["identidade", "Identidade", Store], ["hero", "Página inicial", ImagePlus], ["sobre", "Descrição e sobre", UserRound], ["frota", "Frota", Bike], ["contato", "Contato", MessageCircle], ["aparencia", "Aparência", Palette],
  ];

  return (
    <div className="modalOverlay rentalSiteModalOverlay rentalSiteFullPageV68">
      <div className="recordModal rentalSiteEditorV67 rentalSiteEditorV68">
        <button className="closeModal rentalEditorCloseV68" type="button" onClick={onClose} aria-label="Voltar ao LocaCheck">×</button>
        <header className="rentalEditorHeaderV67">
          ...
          {hasSite && <div className="rentalEditorHeaderActionsV67"><span className={`rentalPublicationBadgeV67 ${site.published ? "published" : "draft"}`}>{site.published ? "PUBLICADO" : "RASCUNHO"}</span><span className="rentalEditorHeaderUrlV70" title={publicUrl}>{publicUrl}</span><button className="btn outline" onClick={copyLink}><Copy size={14}/> Copiar</button><button className="btn outline" onClick={shareLink}><Share2 size={14}/> Compartilhar</button><button className="btn outline" onClick={() => window.open(publicUrl, "_blank", "noopener,noreferrer")} disabled={!site.published}><ExternalLink size={14}/> Ver site</button><button className="btn primary" onClick={handlePublishToggle} disabled={!active || publishing}><Rocket size={14}/> {publishing ? "Publicando..." : site.published ? "Colocar em rascunho" : "Publicar"}</button></div>}
        </header>
        {hasSite && <div className="rentalMobileQuickActionsV71"><button type="button" onClick={onClose}><ArrowLeft size={16}/> Voltar</button><button type="button" onClick={shareLink}><Share2 size={16}/> Compartilhar</button><button type="button" onClick={() => window.open(publicUrl, "_blank", "noopener,noreferrer")} disabled={!site.published}><ExternalLink size={16}/> Abrir site</button></div>}

        {loading ? <div className="rentalSiteLoadingV63"><RefreshCw size={22} className="rentalSiteSpinV63"/> Consultando seu benefício...</div> : !active ? (
          <section className="rentalSiteLockedV67 rentalSiteLockedV68"><LockKeyhole size={42}/><span className="rentalLockedEyebrowV68">MEU SITE TEMPORARIAMENTE BLOQUEADO</span><h3>Seu benefício não está disponível</h3><p>Para preencher, editar ou publicar as informações do seu site, sua conta precisa ter uma compra elegível de créditos nos últimos 30 dias ou uma liberação administrativa ativa.</p>{benefit?.admin_grant_expires_at && <small>Liberação administrativa expirada em {formatDateTime(benefit.admin_grant_expires_at)}.</small>}<button className="btn primary" onClick={() => { onClose?.(); onBuyCredits?.(); }}>Fazer uma recarga para liberar</button></section>
        ) : !hasSite ? (
          <form className="rentalCreateSiteV67" onSubmit={handleCreate}><div className="rentalBenefitCardV63 active"><CheckCircle2 size={25}/><div><span>BENEFÍCIO ATIVO</span><strong>{benefit?.days_remaining || 0} dia(s) restante(s)</strong><p>Válido até {formatDateTime(benefit?.expires_at)}.</p></div><CalendarClock size={20}/></div><div className="rentalCreateGridV67"><label>Nome da locadora *<input value={form.brand_name} onChange={e=>updateField("brand_name",e.target.value)} placeholder="Ex.: Speed Motos" required/></label><label>Endereço do site *<div className="rentalSlugFieldV63"><span className="rentalSlugPrefixV63">loca-check.vercel.app/site/</span><input value={form.slug} onChange={e=>updateField("slug",normalizeRentalSiteSlug(e.target.value))} placeholder="speedmotos" required/></div><small className={slugStatus.available ? "ok" : ""}>{slugStatus.message || "Use letras, números e hífen."}</small></label><label>Slogan<input value={form.tagline} onChange={e=>updateField("tagline",e.target.value)} placeholder="Sua liberdade começa aqui"/></label><label>WhatsApp<div className="rentalWhatsappFieldV72"><span>+55</span><input inputMode="numeric" value={form.whatsapp} onChange={e=>updateField("whatsapp",whatsappLocalBR(e.target.value))} placeholder="88 99999-9999"/></div></label></div>{message && <div className="authMessage">{message}</div>}<button className="btn primary" disabled={saving || slugStatus.checking || slugStatus.available === false}><Rocket size={18}/> {saving ? "Criando..." : "Criar meu site"}</button></form>
        ) : (
          <>
            ...
            <button type="button" className="rentalMobilePreviewButtonV68" onClick={() => setShowMobilePreview(true)}><ExternalLink size={14}/> Prévia em tempo real</button>
            <div className="rentalEditorWorkspaceV68"><div className="rentalEditorBodyV67">
              ...
              {tab === "contato" && <section className="rentalEditorSectionV67"><div className="rentalSectionIntroV67"><MessageCircle/><div><h3>Contato e localização</h3><p>Deixe o cliente encontrar sua locadora e falar rapidamente com você.</p></div></div><div className="rentalEditorGridV67"><label>WhatsApp *<div className="rentalWhatsappFieldV72"><span>+55</span><input inputMode="numeric" value={form.whatsapp} onChange={e=>updateField("whatsapp",whatsappLocalBR(e.target.value))} placeholder="88 99999-9999"/></div></label><label>Telefone<input value={form.phone} onChange={e=>updateField("phone",e.target.value)} placeholder="(88) 3333-3333"/></label><label>E-mail<input value={form.email} onChange={e=>updateField("email",e.target.value)} placeholder="contato@empresa.com"/></label><label>Instagram<input value={form.instagram} onChange={e=>updateField("instagram",e.target.value)} placeholder="https://instagram.com/sualocadora"/></label><label className="wide">Endereço<input value={form.address} onChange={e=>updateField("address",e.target.value)} placeholder="Rua, número, bairro, cidade - UF"/></label><label className="wide">Horário de atendimento<input value={form.opening_hours} onChange={e=>updateField("opening_hours",e.target.value)} placeholder="Seg a Sáb · 08h às 18h"/></label></div></section>}
              ...
              {tab === "frota" && <section className="rentalEditorSectionV67"><div className="rentalSectionIntroV67"><Bike/><div><h3>Sua frota</h3><p>Cadastre modelos, fotos, preços e disponibilidade. Só motos publicadas aparecem no site.</p></div></div><form className="rentalMotoFormV67" onSubmit={saveMoto}><div className="rentalEditorGridV67"><label>Marca/Modelo *<input value={motoForm.name} onChange={e=>setMotoForm({...motoForm,name:e.target.value})} placeholder="Honda CG 160" required/></label><label>Ano<input type="number" min="1980" max="2100" value={motoForm.model_year} onChange={e=>setMotoForm({...motoForm,model_year:e.target.value})} placeholder="2026"/></label><label>Cilindrada (cc)<input type="number" min="0" step="1" value={motoForm.engine_cc} onChange={e=>setMotoForm({...motoForm,engine_cc:e.target.value})} placeholder="160"/></label><label>Caução<input type="number" min="0" step="0.01" value={motoForm.deposit} onChange={e=>setMotoForm({...motoForm,deposit:e.target.value})} placeholder="0,00"/></label><label>Diária<input type="number" min="0" step="0.01" value={motoForm.price_day} onChange={e=>setMotoForm({...motoForm,price_day:e.target.value})}/><small>Deixe em branco caso sua locadora não ofereça essa opção.</small></label><label>Semanal<input type="number" min="0" step="0.01" value={motoForm.price_week} onChange={e=>setMotoForm({...motoForm,price_week:e.target.value})}/><small>Deixe em branco caso sua locadora não ofereça essa opção.</small></label><label>Mensal<input type="number" min="0" step="0.01" value={motoForm.price_month} onChange={e=>setMotoForm({...motoForm,price_month:e.target.value})}/><small>Deixe em branco caso sua locadora não ofereça essa opção.</small></label><label className="wide">Descrição<textarea rows={3} value={motoForm.description} onChange={e=>setMotoForm({...motoForm,description:e.target.value})} placeholder={DEFAULT_MOTO_DESCRIPTION}/></label></div><div className="rentalMotoChecksV67"><label><input type="checkbox" checked={motoForm.available} onChange={e=>setMotoForm({...motoForm,available:e.target.checked})}/> Disponível para locação</label><label><input type="checkbox" checked={motoForm.published} onChange={e=>setMotoForm({...motoForm,published:e.target.checked})}/> Mostrar no site</label></div><ImageField label="Foto da moto" value={motoForm.image_url} onChange={file=>{setUploading("moto"); uploadRentalSiteAsset(site.id,file,"moto").then(url=>setMotoForm(current=>({...current,image_url:url}))).catch(e=>setMessage(e.message)).finally(()=>setUploading(""));}} uploading={uploading==="moto"}/><div className="rentalMotoFormActionsV67"><button type="submit" className="btn primary"><Save size={16}/>{editingMotoId ? "Salvar alterações" : "Adicionar moto"}</button>{editingMotoId && <button type="button" className="btn outline" onClick={()=>{setEditingMotoId(null);setMotoForm(emptyMoto)}}>Cancelar</button>}</div></form><div className="rentalMotoListV67">{motorcycles.length===0 ? <div className="rentalEmptyFleetV67"><Bike size={32}/><strong>Nenhuma moto cadastrada</strong><span>Adicione sua primeira moto acima.</span></div> : motorcycles.map(moto=><article key={moto.id} className="rentalMotoCardV67"><div className="rentalMotoImageV67">{moto.image_url?<img src={moto.image_url} alt={moto.name}/>:<Bike size={30}/>}</div><div className="rentalMotoInfoV67"><strong>{moto.name}</strong><p>{[moto.model_year ? `Ano ${moto.model_year}` : "", moto.engine_cc ? `${moto.engine_cc} cc` : "", moto.deposit > 0 ? `Caução ${money(moto.deposit)}` : ""].filter(Boolean).join(" • ") || moto.description || "Sem informações adicionais"}</p><div className="rentalMotoPricesV67"><b>{money(moto.price_day)}/dia</b><span>{moto.price_week ? `${money(moto.price_week)}/sem` : "Sem preço semanal"}</span><span>{moto.price_month ? `${money(moto.price_month)}/mês` : "Sem preço mensal"}</span></div></div><div className="rentalMotoCardActionsV67"><span className={moto.published?"ok":"muted"}>{moto.published?<><Check size={13}/> no site</>:"rascunho"}</span><button className="btn outline rentalMotoEditButtonV72" onClick={()=>editMoto(moto)}>Editar</button><button className="iconButton" onClick={()=>removeMoto(moto.id)} aria-label="Excluir"><Trash2 size={17}/></button></div></article>)}</div></section>}
            </div>
            <aside className={`rentalLivePreviewV68 ${showMobilePreview ? "isMobilePreviewOpen" : ""}`}>
              <div className="rentalLivePreviewHeaderV68"><div className="rentalLivePreviewHeaderActionsV68"><span className="rentalLivePreviewDotV68">● AO VIVO</span><button type="button" className="rentalMobilePreviewToggleV68" onClick={() => setShowMobilePreview(false)}><ArrowLeft size={15}/> Voltar ao editor</button></div></div>
              <div className="rentalLivePreviewViewportV68">
                <PublicRentalSitePage slug={form.slug || site.slug} previewSite={{ ...site, ...form, whatsapp: whatsappStoredBR(form.whatsapp), published: true }} previewMotorcycles={motorcycles.length ? motorcycles : (motoForm.name ? [{ ...motoForm, id: "draft-preview", published: true }] : [])} />
              </div>
            </aside>
            </div>
